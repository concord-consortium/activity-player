...
context("Test the overall app", () => {
  ...
  describe('Info/Assess (secondary embeddables)',()=>{
    it('verify image question',()=>{
      activityPage.getSecondaryEmbeddable("image-question").eq(0).scrollIntoView()
        .should('be.visible').and('contain',"background image with a Draw Tool button");
    });
    it('verify textbox',()=>{
    ...
    it('verify open response question',()=>{
      activityPage.getSecondaryEmbeddable("open-response-question").scrollIntoView()
        .should('be.visible').and('contain',"open response question");
    });
    it('verify labbook question',()=>{
      activityPage.getSecondaryEmbeddable("labbook-question").scrollIntoView()
        .should('be.visible').and('contain',"Labbook type question");
    });
    ...
    it('verify image interactive',()=>{
      ...
      activityPage.getSecondaryEmbeddable("image-video-interactive").eq(0).find('img')
        .should("have.attr","src","https://media.newyorker.com/photos/5d38c026347b0e000865de67/4:3/w_1555,h_1166,c_limit/Jordan-MadMagazine-1.jpg");
    });
    it('verify video interactive',()=>{
      ...
      activityPage.getSecondaryEmbeddable("image-video-interactive").eq(1).find('video source')
        .should("have.attr","src","https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4");
    });
    it('verify upload image question',()=>{
      activityPage.getSecondaryEmbeddable("image-question").eq(1).scrollIntoView()
        .should('be.visible').and('contain',"Upload image question");
    });
  });
});
