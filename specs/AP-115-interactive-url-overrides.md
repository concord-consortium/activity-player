...
- **Full-URL values and `swap-base` substitution.** The current value model is a single token (e.g. a branch name) matched by `/^[A-Za-z0-9._-]+$/`, substituted into a templated `replace` string. This is a poor fit for use cases where the override value is itself a full base URL whose origin and path replace the matched portion of the original URL while the original's query string and hash are preserved verbatim. The motivating example is PR #548 (AP-72), which adds `?codap=<full-base-url>` to redirect both `codap.concord.org` and `codap3.concord.org` interactives to an arbitrary CODAP build (e.g. `https://codap3.concord.org/staging`) while keeping flag-style query parameters like `interactiveApi` exactly as the activity author wrote them. Supporting this would require two additive features per registry entry: (a) a `valueKind: "url"` mode that relaxes charset validation in favor of structural validation (parses as URL, scheme allowlist), and (b) a `replaceMode: "swap-base"` mode whose output is `base.origin + base.path + (base.query merged with original.query, with original winning on conflicts) + (original.hash || base.hash)`. Both are layered on top of the existing model, not replacements; the current token+template path stays the default.
