...
* noDefaultActivity:                  when set and neither `activity` nor `sequence` is specified, show a dialog that lets the user paste a URL or activity reference from the authoring system instead of falling back to the default sample activity. Accepts an Activity Player URL (forwards its `activity`/`sequence`/`sequenceActivity`/`page` params), a direct authoring JSON endpoint, or a sample activity key.
