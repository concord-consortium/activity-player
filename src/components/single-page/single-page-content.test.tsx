...
describe("Single Page Content component", () => {
  it("renders component", () => {
    const wrapper = mount(<DynamicTextTester><SinglePageContent activity={DefaultTestActivity} pluginsLoaded={true} /></DynamicTextTester>);
    expect(wrapper.find('[data-cy="single-page-content"]').length).toBe(1);
  });
  it("renders component content", () => {
    const wrapper = mount(<DynamicTextTester><SinglePageContent activity={activitySinglePage} pluginsLoaded={true} /></DynamicTextTester>);
    expect(wrapper.find('[data-cy="single-page-content"]').length).toBe(1);
