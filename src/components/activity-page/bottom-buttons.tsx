...
export const BottomButtons: React.FC<BottomButtonProps> = (props) => {
  return (
    <div className="bottom-buttons">
      <div>
        { <button className="button" disabled={!props.onBack} onClick={props.onBack} data-cy="bottom-button-back">{"< Back"}</button> }
      </div>
      <div>
      ...
      <div>
        { <button className="button" disabled={!props.onNext} onClick={props.onNext} data-cy="bottom-button-next">{"Next >"}</button> }
      </div>
    </div>
  );
};
